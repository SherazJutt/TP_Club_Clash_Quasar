...
<script setup>
  ...
  const Rome = [
    ...
    },
    {
      ...
      "Trackvariant": "St Peter's Kickoff",
      "OpponentClub": "UERK",
      "OpponentPlayer": "BingusTheDinoCat",
      "CarClass": "D",
  ...
  let trackn = 'OpponentPlayer'
  // const addtrackdata = (() => {
  getDoc(doc(db, 'management_data', 'Tracks')).then((data) => {
    // console.log(data.data().Cairo);
    let each = data.data().Cairo
    each.forEach(element => {
      console.log(element[trackn]);
    });
  })
  // })
  </script>
